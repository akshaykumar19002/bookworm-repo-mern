...
class Main extends Component {
  ...
  rent() {
    ...
    if (userId == null) {
      ...
      .then((rentObj) => {
        if (rentObj !== undefined && rentObj !== null && rentObj.error === undefined) {
          ...
          .then((billing) => {
            ...
            if (billing !== undefined && billing !== null && billing.error === undefined)
    ...
    .then(data=> 
      ...
      (data) => {
        this.setState({books: data.filter(ebook => ebook.actions.includes('buy') || ebook.actions.includes('rent'))
              .map(ebook => (
          <div className="col" key={ebook._id} style={{ marginBottom:"3rem" }}>
            <Card style={{ width: '20rem' }}>
              <a className="viewlink" href={"/view/" + ebook._id}>
                  ...
                  <div className="row button-content">
                    {
                      ebook.actions.includes('buy') ?
                        <Button className="col custom-button" onClick={this.buy.bind(this, ebook._id)} style={{marginTop: "5%",marginRight:"1rem"}} variant="secondary">Buy</Button>
                      : <></>
                    }
                    {
                      ebook.actions.includes('rent') ?
                        <Button className="col custom-button" onClick={this.showRentModal.bind(this, ebook._id, ebook.price)} style={{marginTop: "5%",marginRight:"1rem"}} variant="secondary">Rent</Button>
                      : <></>
                    }
                    <Button className="col custom-button" onClick={this.showCartModal.bind(this, ebook._id, ebook.price, ebook.actions)} style={{marginTop: "5%"}} variant="secondary">+ Cart</Button>
