...
class Lendinglibrary extends Component {
  ...
  fetchBooks() {
    ...
    fetch(process.env.REACT_APP_BACKEND_URL + url)
        ...
        .then((result) => { this.setState({ebooks: 
          ...
          ?.map((ebook) => (
          <div className="col" key={ebook._id}>
            <Card style={{ width: '18rem' }}>
              ...
              <Card.Body>
                ...
                <Button className="col" onClick={this.packageModel.bind(this, ebook._id)} style={{marginTop: "5%",marginRight:"1rem", width: '100%'}} variant="secondary">Lend</Button>
              </Card.Body>
            </Card>
          </div>
          ))
  ...
  lendBook() {
    ...
    let pkg = this.state.packages.filter((pack) => {return pack._id === this.state.selectedPackage})[0]
    let lendObject = {
          ...
          <div className="row">
            {
              (this.state.packages?.length > 0) ? this.state.packages.map((pkg)=> (
                <div key={pkg._id} className="col" onClick={this.updatePackageSelection.bind(this, pkg._id)}>
                  <Card style={{ width: '15rem' }}>
                  <Card.Body>
                      <input type="radio" value={pkg._id}
                      checked={this.state.selectedPackage === pkg._id}
